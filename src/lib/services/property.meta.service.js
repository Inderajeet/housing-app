...
export async function getPropertyMeta(identifier) {
  ...
  // Also try matching the raw identifier (for numeric property_id fallback)
  ...
  const rows = await prisma.$queryRawUnsafe(
    `SELECT
       ...
       (SELECT file_url FROM property_assets
        WHERE property_id = p.property_id AND asset_type = 'image'
        ORDER BY asset_id ASC LIMIT 1) AS primary_image
     FROM properties p
