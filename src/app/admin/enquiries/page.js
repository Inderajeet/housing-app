...
import { getEnquiries, createEnquiry, updateEnquiry, getSaleProperties, getRentProperties } from '@/lib/adminApi';
import { adminApi } from '@/lib/adminApi';

const EMPTY_FORM = { property_type: 'sale', property_id: '', buyer_name: '', buyer_phone: '', contacted: false };
...
export default function EnquiriesPage() {
  ...
  const handleViewProperty = async (e, event) => {
    ...
    setPropLoading(true);
    try {
      const endpoint = e.property_type === 'rent' ? `/rent/${e.property_id}` : `/sale/${e.property_id}`;
  ...
  const columns = useMemo(
    () => [
      ...
      {
        header: 'Phone',
        accessor: (e) => <div className="font-mono text-sm">{e.buyer_phone || e.seller_phone || e.contact_phone || 'N/A'}</div>,
      ...
      },
      {
        ...
        accessor: (e) => (
          <div className="flex items-center gap-2">
            <button onClick={(ev) => { ev.stopPropagation(); handleView(e); }} className="p-2 hover:bg-gray-100 rounded-lg text-gray-600 transition-colors" title="View Enquiry">
            ...
            <button
              onClick={(ev) => {
                ...
                const msg = encodeURIComponent(`Hi ${name}, regarding your enquiry for ${e.title || 'property'}...`);
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white p-6 rounded-3xl shadow-sm border border-gray-100 space-y-4">
        <div className="flex flex-wrap gap-4 items-end">
            ...
            <div className="flex gap-2">
              {['buyer', 'seller'].map((t) => (
                <button
                  ...
                  className={`px-4 py-1.5 rounded-lg text-[10px] font-bold uppercase tracking-wider transition-all ${enquiryType === t ? 'bg-blue-600 text-white shadow-lg shadow-blue-200' : 'bg-gray-100 text-gray-500'}`}
                >
                  {t}
                </button>
              ))}
          ...
          <button onClick={() => setFilters({ dateRange: 'all', startDate: '', endDate: '' })} className="text-[10px] font-bold text-red-500 uppercase pb-3 hover:underline">
            Reset
          </button>
        </div>
            ...
            <div className="p-8 space-y-6">
              ...
              <div className="p-4 rounded-xl border bg-gray-50">
                ...
                <p className="text-xs text-gray-500 mt-1">
                  {selected.property_type?.toUpperCase() || 'N/A'} · {selected.amount ? `₹${Number(selected.amount).toLocaleString()}` : 'Price N/A'}
                </p>
              </div>
              <div>
                ...
                <div className="flex gap-2">
                  {[true, false].map((v) => (
                    <button
                      ...
                      className={`px-4 py-1.5 rounded-lg text-[10px] font-bold uppercase transition-all ${contacted === v ? (v ? 'bg-emerald-600 text-white' : 'bg-red-500 text-white') : 'bg-gray-100 text-gray-500'}`}
                    >
                      {v ? 'Yes' : 'No'}
                    </button>
                  ))}
                ...
                <div className="space-y-5">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <span className={`inline-block px-2 py-0.5 rounded text-[10px] font-bold uppercase ${propData.status === 'approved' ? 'bg-emerald-100 text-emerald-700' : propData.status === 'pending' ? 'bg-amber-100 text-amber-700' : 'bg-gray-100 text-gray-600'}`}>
                        {propData.status || '—'}
                      </span>
                    </div>
                    <div>
                      <p className="text-[10px] font-bold uppercase tracking-widest text-gray-400">{propData.rent_status ? 'Rent Status' : 'Sale Status'}</p>
                      <p className="font-semibold">{propData.rent_status || propData.sale_status || '—'}</p>
                    </div>
                  </div>
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      <p className="text-[10px] font-bold uppercase tracking-widest text-gray-400">{propData.rent_amount ? 'Rent/Month' : 'Price'}</p>
                      <p className="font-bold text-lg">₹{Number(propData.rent_amount || propData.price || 0).toLocaleString()}</p>
                    </div>
                    <div>
                      <p className="text-[10px] font-bold uppercase tracking-widest text-gray-400">{propData.bhk ? 'BHK' : 'Sale Type'}</p>
                  ...
                  <div>
                    ...
                    <p className="font-semibold text-sm">
                      {[propData.village_name, propData.taluk_name, propData.district_name].filter(Boolean).join(', ') || propData.address || '—'}
                    </p>
                  </div>
                </div>
              ) : null}
