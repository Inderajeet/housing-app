...
export default function RentOwnersPage() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try { const res = await getSellers('rent'); setSellers(res.data || []); } catch { setSellers([]); }
    setLoading(false);
  }, []);
  ...
  const openModal = async (seller, mode) => {
    ...
    setProperties([]); setError(''); setModal(mode);
  ...
  const renderRowActions = useCallback((r) => (
    <>
      {r.phone_number && (
        <a
          href={`https://wa.me/${(r.phone_number || '').replace(/\D/g, '')}`}
          target="_blank"
      ...
      <button
        onClick={(e) => {
          e.stopPropagation();
          setNotesTarget({
            title: r.name || r.phone_number || `Owner #${r.seller_id}`,
            params: { seller_id: r.seller_id },
          });
        }}
  ...
  return (
    <div className="space-y-6">
      ...
      {loading ? <Loader text="Loading rent owners..." /> : (
        <DataTable columns={columns} data={filtered} onEdit={row => openModal(row, 'edit')} onView={row => openModal(row, 'view')} actions={renderRowActions} />
      )}
        ...
        <div className="!m-0 fixed inset-0 bg-black/40 backdrop-blur-sm z-50 flex items-center justify-center p-4 md:p-10">
          <div className="bg-white rounded-3xl shadow-2xl w-full max-w-2xl max-h-full flex flex-col overflow-hidden">
            <div className="flex items-center justify-between px-8 py-5 border-b bg-gray-50/50">
              <h2 className="text-xl font-bold uppercase tracking-tight text-gray-800">{modal === 'add' ? 'Add Rent Owner' : isReadOnly ? 'Owner Details' : 'Edit Owner'}</h2>
