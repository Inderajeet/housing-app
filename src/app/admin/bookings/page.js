...
import { getBookings, getContactNotes, createContactNote, deleteContactNote } from '@/lib/adminApi';
...
const getUnitTypeStyle = (type) => {
  if (type === 'rent') return 'bg-emerald-100 text-emerald-700';
  if (type === 'sale' || type === 'plot') return 'bg-blue-100 text-blue-700';
...
const dropdownClass =
  'px-4 py-2.5 bg-gray-50 border border-gray-200 rounded-xl text-xs font-bold uppercase outline-none focus:ring-2 focus:ring-blue-500/20 transition-all';
...
export default function BookingsPage() {
  ...
  const loadData = useCallback(async () => {
    setLoading(true);
    try {
      const unitTypeFilter = filters.unit_type !== 'all' ? filters.unit_type : null;
      const statusFilter = filters.status !== 'all' ? filters.status : null;
      const response = await getBookings(unitTypeFilter, statusFilter);
  ...
  const handleExport = () => {
    const dataToExport = filteredBookings.map((b) => ({
      ...
      'Unit Type': getUnitTypeLabel(b.unit_type),
      'Buyer': b.buyer_name || b.buyer_phone || '-',
      'Phone': b.buyer_phone || '-',
      'Status': b.status?.toUpperCase() || '-',
      'Token Amount': b.token_amount ? formatCurrency(b.token_amount) : '-',
      'Booking Date': formatDateShort(b.locked_at),
      'Expiry Date': formatDateShort(b.expires_at),
    }));
    const ws = XLSX.utils.json_to_sheet(dataToExport);
    const wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws, 'Bookings_Data');
    XLSX.writeFile(wb, `Bookings_All_${new Date().toLocaleDateString()}.xlsx`);
  };
  ...
  const columns = useMemo(
    () => [
      {
        header: 'Booking ID',
        accessor: (b) => `BK-${b.booking_id}`,
        className: 'font-semibold text-blue-600 font-mono text-xs',
      },
      {
        header: 'Property ID',
        accessor: (b) => b.formatted_id || b.property_id,
        ...
        accessor: (b) => (
          <div>
            <div className="font-medium text-sm">
              ...
              {b.buyer_name && b.buyer_phone && (
                <span className="text-xs font-mono text-gray-500 ml-1">({b.buyer_phone})</span>
              )}
            </div>
          </div>
        ),
      ...
      },
      {
        ...
        accessor: (b) => {
          const status =
            BOOKING_STATUSES.find((s) => s.value === b.status) || {
              label: b.status?.toUpperCase() || '-',
              color: 'bg-gray-100 text-gray-800',
            };
          return (
            <span className={`px-2 py-1 rounded-lg text-[10px] font-bold uppercase ${status.color}`}>
              {status.label}
            </span>
          );
        },
      ...
      {
        header: 'Token Amount',
        accessor: (b) => formatCurrency(b.token_amount),
        className: 'font-semibold',
      },
      {
        header: 'Booking Date',
        accessor: (b) => formatDateShort(b.locked_at),
      ...
      },
      {
        ...
        accessor: (b) => (
          <div className="flex items-center gap-2">
            <button
              onClick={(e) => { e.stopPropagation(); handleView(b); }}
            ...
            {(b.buyer_phone || b.buyer?.phone_number) && (
              <a
                href={`https://wa.me/${(b.buyer_phone || b.buyer?.phone_number || '').replace(/\D/g, '')}`}
                target="_blank"
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white p-6 rounded-3xl shadow-sm border border-gray-100 space-y-4">
        <div className="flex flex-wrap gap-4 items-end">
          ...
          <button
            onClick={() => setFilters({ dateRange: 'all', startDate: '', endDate: '', status: 'all', unit_type: 'all' })}
      ...
      {loading ? (
        ...
        <div className="overflow-x-auto">
          <DataTable columns={columns} data={filteredBookings} emptyMessage="No bookings found" onRowClick={handleView} />
        </div>
      )}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm p-4 md:p-10">
          <div className="bg-white rounded-3xl w-full max-w-2xl shadow-2xl overflow-hidden flex flex-col max-h-full">
            ...
            <div className="p-8 overflow-y-auto flex-1 space-y-6">
              <div className="grid grid-cols-2 gap-6">
                <div className="flex flex-col space-y-1">
                  ...
                  <p className="font-bold font-mono text-lg text-blue-700">BK-{selected.booking_id}</p>
                </div>
                <div className="flex flex-col space-y-1">
                  <p className="text-[10px] font-bold uppercase tracking-widest text-gray-400">Property ID</p>
                  <p className="font-semibold font-mono text-gray-800">{selected.formatted_id || selected.property_id}</p>
                </div>
              </div>
              ...
              <div className="p-5 bg-gradient-to-br from-gray-50 to-white rounded-2xl border border-gray-200 space-y-3">
                ...
                {selected.buyer_name && selected.buyer_phone && (
                  <p className="text-sm font-mono text-gray-600">{selected.buyer_phone}</p>
                )}
              </div>
              ...
              <div className="grid grid-cols-2 gap-6">
                ...
                <div className="flex flex-col space-y-1">
                  <p className="text-[10px] font-bold uppercase tracking-widest text-gray-400">Token Amount</p>
                  <p className="text-xl font-bold text-gray-900">{formatCurrency(selected.token_amount)}</p>
                </div>
              </div>

              <div className="grid grid-cols-2 gap-6">
                <div className="flex flex-col space-y-1">
                  <p className="text-[10px] font-bold uppercase tracking-widest text-gray-400">Booking Date</p>
                  <p className="font-semibold text-gray-800">{formatDate(selected.locked_at)}</p>
                </div>
                <div className="flex flex-col space-y-1">
                  <p className="text-[10px] font-bold uppercase tracking-widest text-gray-400">Expiry Date</p>
                  <p className="font-semibold text-gray-800">{formatDate(selected.expires_at) || '-'}</p>
                </div>
              </div>
            </div>
            ...
            <div className="px-8 py-6 border-t flex justify-between items-center bg-gray-50 shrink-0">
              ...
              <button onClick={() => setIsViewOpen(false)} className="px-6 py-2 rounded-xl border border-gray-300 font-bold text-xs uppercase text-gray-600 hover:bg-gray-100 transition-colors">
                Close
              </button>
            </div>
          </div>
