...
export default async function Image({ params }) {
  ...
  // Pre-fetch property image as base64 to avoid Satori external URL issues
  const rawImageUrl = p.primary_image || DEFAULT_IMAGE;
  const imageData = await fetchImageAsDataUrl(rawImageUrl);
  ...
  return new ImageResponse(
    <div style={{ display: 'flex', flexDirection: 'column', width: '100%', height: '100%', background: '#f8fafc', fontFamily: 'sans-serif' }}>
      ...
      <div style={{ display: 'flex', width: '100%', height: 346, overflow: 'hidden', background: '#e2e8f0' }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', flex: 1, padding: '14px 40px 12px', background: '#f0fdf4' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 2 }}>
          <div style={{ display: 'flex', width: 8, height: 8, background: '#3b82f6', borderRadius: 4, flexShrink: 0 }} />
          {locationStr ? <span style={{ display: 'flex', fontSize: 15, color: '#475569', fontWeight: 600 }}>{locationStr}</span> : null}
          {locationStr && layoutOrLandmark ? <span style={{ display: 'flex', fontSize: 15, color: '#cbd5e1', margin: '0 2px' }}>|</span> : null}
        ...
        {/* Ratings */}
        <div style={{ display: 'flex', gap: 0, borderRadius: 8, overflow: 'hidden', border: '1px solid #cbd5e1', marginBottom: 10 }}>
          {!isRent ? ratingCell('#24675e', 'Legal', 'grade', legalGrade) : null}
          {!isRent ? ratingCell('#235bd8', 'Area Sales', 'speed', areaSalesSpeed) : null}
